...
printf "${outmagenta}Backup directory:${outdefault} $restoretarget\n"
...
cp "$restoretarget"/query_ip_whitelist.txt "$installdir"/teamspeak-server-current/
...
cp "$restoretarget"/query_ip_blacklist.txt "$installdir"/teamspeak-server-current/
...
cp "$restoretarget"/ts3server.sqlitedb "$installdir"/teamspeak-server-current/
...
cp -R "$restoretarget"/files /"$installdir"/teamspeak-server-current/
...
cp "$restoretarget"/*.ini "$installdir"/teamspeak-server-current/
...
cp "$restoretarget"/licensekey.dat "$installdir"/teamspeak-server-current/
...
cp "$restoretarget"/tsdns/tsdns_settings.ini "$installdir"/teamspeak-server-current/tsdns/
